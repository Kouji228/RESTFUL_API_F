 ...
 *   get:
 *     summary: 獲取所有使用者
 *     description: 獲取系統中所有使用者的基本資訊（不包含密碼）
 *     tags: [使用者管理]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: 成功獲取所有使用者
 *         content:
 *           application/json:
 *             schema:
 *               type: object
...
router.get('/', async (req, res) => {
  try {
    ...
    // 確保狀態碼是有效的整數
    let statusCode = 500;
    if (error.code && !isNaN(parseInt(error.code))) {
 ...
 *   post:
 *     summary: 註冊新使用者
 *     description: 建立新的使用者帳號，包含密碼加密和自動頭像生成
 *     tags: [使用者管理]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *   post:
 *     summary: 使用者登入
 *     description: 驗證使用者帳號密碼並回傳 JWT token
 *     tags: [使用者認證]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *   post:
 *     summary: 檢查登入狀態
 *     description: 驗證當前 token 並回傳新的有效 token
 *     tags: [使用者認證]
 *     security:
 *       - bearerAuth: []
 *     responses:
